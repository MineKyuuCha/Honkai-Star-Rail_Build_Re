...
title_text = (
    ...
    if lang == "English"
    else "Rekomendasi Build Karakter Honkai Star Rail"
)

st.title(title_text)

st.session_state.user_input = st.text_input(
    smart_translate("Input Comment / ask build"),
...
if st.session_state.user_input:
    ...
    if top_match["similarity"] < 0.1:
        ...
        st.warning(
            smart_translate("You sure not typo ?")
        )
        ...
        st.subheader(
            smart_translate(
                f"Recommended build for: {top_match['karakter']} "
            )
        )

        st.image(
            top_match["gambar_karakter"],
            width=700
        )

        st.markdown(
            f"<div class='big-font'>{top_match['karakter']} ⭐⭐⭐⭐⭐</div>",
        ...
        with col3:
            st.write(
                smart_translate("Type:"),
                top_match["tipe_build"]
            )

        st.subheader(
            smart_translate("Build Description")
        )

        deskripsi = top_match["deskripsi_build"]

        st.write(
            smart_translate(deskripsi)
        )
        ...
        st.image(
            top_match["gambar_planar"],
            width=100
        )

        st.subheader(
            smart_translate("Lightcone")
        )
        ...
        st.image(
            top_match["gambar_lightcone"],
            width=180
        )

        st.subheader(
            smart_translate("Team Recommendation")
        )

        team_imgs = (
            top_match["gambar_team_rekomendasi"]
            .split(";")
        )

        team_names = (
            top_match["team_rekomendasi"]
            .split(";")
        )
